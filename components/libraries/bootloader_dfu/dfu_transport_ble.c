...
#define DEVICE_NAME                          "1502OTA" //"DfuTarg"                                               /**< Name of device. Will be included in the advertising data. */
...
static void advertising_start(void)
{
    if (!m_is_advertising)
    {
            ...
            if (memcmp(m_ble_peer_data.irk.irk, empty_irk.irk, sizeof(empty_irk.irk)) == 0)
            {
              ...
              m_adv_params.fp          = BLE_GAP_ADV_FP_ANY;
               m_adv_params.interval    = 0;
                m_adv_params.timeout     = 0;*///kevin delete
                ...
                m_adv_params.timeout     = BLE_GAP_ADV_TIMEOUT_LIMITED_MAX;
            }
            else
            {
        ...
        //nrf_gpio_pin_clear(ADVERTISING_LED_PIN_NO);//kevin delete
...
static void on_ble_evt(ble_evt_t * p_ble_evt)
{
    ...
    switch (p_ble_evt->header.evt_id)
    {
        case BLE_GAP_EVT_CONNECTED:
            //nrf_gpio_pin_clear(CONNECTED_LED_PIN_NO);//kevin delete
            //nrf_gpio_pin_set(ADVERTISING_LED_PIN_NO);//kevin delete
        ...
        case BLE_GAP_EVT_DISCONNECTED:
            {
                ...
                //nrf_gpio_pin_set(CONNECTED_LED_PIN_NO);//kevin delete
...
//#if DEBUG_BOOTLOADER_EN
   //  unsigned char debug_data[6]={0,0,0,0,0,0};
//#endif
//    leds_init();//kevin delete
    ...
    //advertising_stop();//kevin add 151225
...
simple_uart_init();
   ...
   /* if (err_code == NRF_SUCCESS)
    {
        m_ble_peer_data_valid = true;//kevin delete
        //err_code = sd_ble_gap_address_set(BLE_GAP_ADDR_CYCLE_MODE_NONE, &(m_ble_peer_data.addr));
        //APP_ERROR_CHECK(err_code);      
#if DEBUG_BOOTLOADER_EN
      DbgPrintf_str("peer_data_get sucess,address\r\n");
      DbgPrintf_hex(m_ble_peer_data.addr.addr[0]);
      DbgPrintf_hex(m_ble_peer_data.addr.addr[1]);
      DbgPrintf_hex(m_ble_peer_data.addr.addr[2]);
      DbgPrintf_hex(m_ble_peer_data.addr.addr[3]);
      DbgPrintf_hex(m_ble_peer_data.addr.addr[4]);
      DbgPrintf_hex(m_ble_peer_data.addr.addr[5]);
      //simple_uart_putstringbuff((uint8_t *)m_ble_peer_data.addr.addr,6);
#endif
    }
    else*/
    {
    ...
    etSpim1NorFlashInit(); 
    etSpim1ReadNorFlashStdMode(KEYID_START_ADDR,keyid,16);
 #if DEBUG_BOOTLOADER_EN
      //simple_uart_putstringbuff(keyid+TAG1_ADDR_OFFSET,9);
 #endif
 ...
 #if DEBUG_BOOTLOADER_EN
        DbgPrintf_str("get SPI data invalid1\r\n");
 #endif        
      }
      ...
      && (keyid[MAC_ADDRESS_ADDR_OFFSET+5]==0xff))
      {
        //memcpy(t_addr.addr,BD_ADDR,6);
  #if DEBUG_BOOTLOADER_EN
        DbgPrintf_str("get SPI data invalid2\r\n");
 #endif
      }
      else
      {        
        memcpy(addr.addr,keyid+MAC_ADDRESS_ADDR_OFFSET,MAC_ADDRESS_LENGTH);
 #if DEBUG_BOOTLOADER_EN
        DbgPrintf_str("get SPI data valid\r\n");
 #endif
      }
    ...
    {
 #if DEBUG_BOOTLOADER_EN
      DbgPrintf_str("ID data not exist\r\n");
 #endif     
    }

#endif      
        //addr.addr[0] += 1;//kevin add
        err_code = sd_ble_gap_address_set(BLE_GAP_ADDR_CYCLE_MODE_NONE, &addr);
        APP_ERROR_CHECK(err_code);
    }
    ...
    services_init();
    conn_params_init();
    sec_params_init();//kevin delete
    advertising_start();
 #if DEBUG_BOOTLOADER_EN
        DbgPrintf_str("dfu start advertising\r\n");
 #endif 
    return NRF_SUCCESS;
}
